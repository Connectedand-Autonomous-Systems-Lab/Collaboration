...
public class MazeGenHuman : MonoBehaviour
{
    ...
    private float victimYPos = -3.432f;
    ...
    void SaveGeneratedMaze(){
        GameObject generated = this.gameObject;
        if(generated != null){
            generated.transform.SetParent(mazeGO.transform);
        }
    }
    ...
    private void GenerateRandomWalkablePoints()
    {
        ...
        GameObject victims = new GameObject("Victims");
        victims.transform.SetParent(mazeGO.transform);
        for (int i = 0; i < pointCount && walkableSpaces.Count > 0; i++)
        {
            ...
            GameObject vict = Instantiate(victimModel, worldPosition, Quaternion.identity);
            vict.transform.SetParent(victims.transform);
        }
        ...
        Instantiate(tb3, worldPos + new Vector3(1f, 0, 0), Quaternion.identity);
        GameObject avt = Instantiate(avatar, worldPos + new Vector3(-1f, 0, 0), Quaternion.identity);
        // avt.transform.SetParent(mazeGO.transform);
